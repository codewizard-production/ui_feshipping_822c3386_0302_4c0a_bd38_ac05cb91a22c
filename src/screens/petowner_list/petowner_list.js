...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPetOwnersMulti(query, "").then(async (resP) => {
            if (resP.status) {
                for (let i = 0; i < resP.values.length; i++) {
                    let _PetOwner = resP.values[i];
                    let _row = {
                        prop1: _PetOwner.OwnerName,
                        prop2: _PetOwner.ContactPrimary,
                        prop3: _PetOwner.Pincode,
                    };
