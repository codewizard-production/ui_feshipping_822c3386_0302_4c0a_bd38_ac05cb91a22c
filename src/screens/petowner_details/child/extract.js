import PetOwnerJsonConfig from "config/petowner_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "petowner", expand: "", exclude: [], func: Support.AddOrUpdatePetOwner }
]

const FetchPetOwnerInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(PetOwnerJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of PetOwnerJsonConfig[elm]) {
...
const FetchPetOwnerDetails = async (petownerId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (petownerId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetPetOwnerSingle(petownerId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetPetOwnerSingle(petownerId);
            }
            
            if (rslt.status) {

                const petowner = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? petowner : petowner[source];
...
const Extract = async (petownerId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchPetOwnerInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(petownerId)) {
                    await FetchPetOwnerDetails(petownerId, enums).then(({ row, backRow }) => {
