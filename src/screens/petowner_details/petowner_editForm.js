...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], petOwnerId, petowner, numfields;

        petowner = row['petowner'];
        petOwnerId = row['petowner'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) petOwnerId = rslt.id;
                            ...
                            rslt = await Support.AddOrUpdatePetOwner(data, dropDownOptions);
        ...
        global.AlertPopup("success", "PetOwner is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
