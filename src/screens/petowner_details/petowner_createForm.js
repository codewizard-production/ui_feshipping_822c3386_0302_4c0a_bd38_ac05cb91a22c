...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, petOwnerId, numfields;
        const mapItems = MapItems;

        let petowner = row['petowner'];

        numfields = Helper.GetAllNumberFields(petowner);
        if (numfields.length > 0) Helper.UpdateNumberFields(petowner, numfields);


        // Add Or Update PetOwner
        rslt = await Support.AddOrUpdatePetOwner(petowner, []);
        if (rslt.status) {
            petOwnerId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the PetOwner and navigation entity if it is deos not exist
            let navItem = petowner.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "OwnerId", value: parseInt(petOwnerId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdatePetOwner(data);
        ...
        global.AlertPopup("success", "PetOwner is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
